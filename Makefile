...
OPENBSD_SITE ?= https://cdn.openbsd.org/pub/OpenBSD
...
# NetBSD architecture mapping
# NetBSD uses: amd64, evbarm-aarch64, i386, sparc64, evbppc
NETBSD_ARCH_DIR = $(if $(NETBSD_VER),$(if $(findstring x86_64,$(TARGET)),netbsd-$(NETBSD_VER)-amd64,$(if $(findstring aarch64,$(TARGET)),netbsd-$(NETBSD_VER)-evbarm-aarch64,$(if $(findstring i386,$(TARGET)),netbsd-$(NETBSD_VER)-i386,$(if $(findstring i586,$(TARGET)),netbsd-$(NETBSD_VER)-i386,$(if $(findstring i686,$(TARGET)),netbsd-$(NETBSD_VER)-i386,$(if $(findstring sparc64,$(TARGET)),netbsd-$(NETBSD_VER)-sparc64)))))))

# OpenBSD architecture mapping
# OpenBSD uses: amd64, arm64, armv7, i386, riscv64, powerpc64, sparc64
OPENBSD_ARCH_DIR = $(if $(OPENBSD_VER),$(if $(findstring x86_64,$(TARGET)),openbsd-$(OPENBSD_VER)-amd64,$(if $(findstring aarch64,$(TARGET)),openbsd-$(OPENBSD_VER)-arm64,$(if $(findstring armv7,$(TARGET)),openbsd-$(OPENBSD_VER)-armv7,$(if $(findstring i386,$(TARGET)),openbsd-$(OPENBSD_VER)-i386,$(if $(findstring i586,$(TARGET)),openbsd-$(OPENBSD_VER)-i386,$(if $(findstring i686,$(TARGET)),openbsd-$(OPENBSD_VER)-i386,$(if $(findstring riscv64,$(TARGET)),openbsd-$(OPENBSD_VER)-riscv64,$(if $(findstring powerpc64,$(TARGET)),openbsd-$(OPENBSD_VER)-powerpc64,$(if $(findstring sparc64,$(TARGET)),openbsd-$(OPENBSD_VER)-sparc64))))))))))
...
srcclean:
	( cd $(CURDIR) && ( chmod -R u+rwX netbsd-* openbsd-* 2>/dev/null || true ) && rm -rf sources gcc-* binutils-* musl-* glibc-* gmp-* mpc-* mpfr-* isl-* build build-* linux-* mingw-w64-* freebsd-* netbsd-* openbsd-* llvm-project-* zlib-* zstd-* libxml2-* )
...
# OpenBSD source download rules
# Downloads baseXX.tgz and compXX.tgz separately with hash verification
# $(1) = architecture suffix (e.g., amd64, arm64, i386)
# $(2) = download path (e.g., amd64, arm64, i386)
define OPENBSD_DOWNLOAD_RULE
$$(SOURCES)/openbsd-%-$(1)-base.tgz: hashes/openbsd-%-$(1)-base.tgz.sha1 | $$(SOURCES)
	mkdir -p $$@.tmp
	cd $$@.tmp && $$(DL_CMD) $$(notdir $$@) $$(OPENBSD_SITE)/$$*/$(2)/base$$(subst .,,$$*).tgz
	cd $$@.tmp && touch $$(notdir $$@)
	cd $$@.tmp && $$(SHA1_CMD) $$(CURDIR)/hashes/$$(notdir $$@).sha1
	mv $$@.tmp/$$(notdir $$@) $$@
	rm -rf $$@.tmp

$$(SOURCES)/openbsd-%-$(1)-comp.tgz: hashes/openbsd-%-$(1)-comp.tgz.sha1 | $$(SOURCES)
	mkdir -p $$@.tmp
	cd $$@.tmp && $$(DL_CMD) $$(notdir $$@) $$(OPENBSD_SITE)/$$*/$(2)/comp$$(subst .,,$$*).tgz
	cd $$@.tmp && touch $$(notdir $$@)
	cd $$@.tmp && $$(SHA1_CMD) $$(CURDIR)/hashes/$$(notdir $$@).sha1
	mv $$@.tmp/$$(notdir $$@) $$@
	rm -rf $$@.tmp

openbsd-%-$(1): $$(SOURCES)/openbsd-%-$(1)-base.tgz $$(SOURCES)/openbsd-%-$(1)-comp.tgz
	( chmod -R u+rwX $$@.tmp $$@ 2>/dev/null || true ) && rm -rf $$@.tmp $$@
	mkdir -p $$@.tmp
	cd $$@.tmp && tar -xzf $$(CURDIR)/$$(SOURCES)/openbsd-$$*-$(1)-base.tgz
	cd $$@.tmp && tar -xzf $$(CURDIR)/$$(SOURCES)/openbsd-$$*-$(1)-comp.tgz
	mv $$@.tmp $$@
endef

# Generate download rules for each OpenBSD architecture
$(eval $(call OPENBSD_DOWNLOAD_RULE,amd64,amd64))
$(eval $(call OPENBSD_DOWNLOAD_RULE,arm64,arm64))
$(eval $(call OPENBSD_DOWNLOAD_RULE,armv7,armv7))
$(eval $(call OPENBSD_DOWNLOAD_RULE,i386,i386))
$(eval $(call OPENBSD_DOWNLOAD_RULE,riscv64,riscv64))
$(eval $(call OPENBSD_DOWNLOAD_RULE,powerpc64,powerpc64))
$(eval $(call OPENBSD_DOWNLOAD_RULE,sparc64,sparc64))
...
%: %.orig | $(SOURCES)/config.sub $(SOURCES)/config.guess
	...
	if [ -d patches/$@ ] && [ -n "$(shell find patches/$@ -type f)" ]; then \
		if [ -n "$(findstring mingw,$(TARGET))" ]; then \
			cat $(filter-out %-musl.diff %-gnu.diff %-freebsd.diff %-netbsd.diff %-openbsd.diff %-nonmingw.diff,$(wildcard patches/$@/*)) | ( cd $@.tmp && $(COWPATCH) -p1 ); \
		elif [ -n "$(findstring freebsd,$(TARGET))" ]; then \
			cat $(filter-out %-mingw.diff %-gnu.diff %-musl.diff %-netbsd.diff %-openbsd.diff %-nofreebsd.diff,$(wildcard patches/$@/*)) | ( cd $@.tmp && $(COWPATCH) -p1 ); \
		elif [ -n "$(findstring netbsd,$(TARGET))" ]; then \
			cat $(filter-out %-mingw.diff %-gnu.diff %-musl.diff %-freebsd.diff %-openbsd.diff %-nonetbsd.diff,$(wildcard patches/$@/*)) | ( cd $@.tmp && $(COWPATCH) -p1 ); \
		elif [ -n "$(findstring openbsd,$(TARGET))" ]; then \
			cat $(filter-out %-mingw.diff %-gnu.diff %-musl.diff %-freebsd.diff %-netbsd.diff %-noopenbsd.diff,$(wildcard patches/$@/*)) | ( cd $@.tmp && $(COWPATCH) -p1 ); \
		elif [ -n "$(findstring musl,$(TARGET))" ]; then \
			cat $(filter-out %-mingw.diff %-gnu.diff %-freebsd.diff %-netbsd.diff %-openbsd.diff %-nonmusl.diff,$(wildcard patches/$@/*)) | ( cd $@.tmp && $(COWPATCH) -p1 ); \
		else \
			cat $(filter-out %-mingw.diff %-musl.diff %-freebsd.diff %-netbsd.diff %-openbsd.diff %-nongnu.diff,$(wildcard patches/$@/*)) | ( cd $@.tmp && $(COWPATCH) -p1 ); \
		fi \
	fi
...
ifeq ($(COMPILER),clang)
extract_all: | $(filter-out mingw-w64-% glibc-% freebsd-% netbsd-% openbsd-%,$(SRC_DIRS))
else ifneq ($(findstring mingw,$(TARGET)),)
extract_all: | $(filter-out linux-% musl-% glibc-% freebsd-% netbsd-% openbsd-%,$(SRC_DIRS))
else ifneq ($(findstring freebsd,$(TARGET)),)
extract_all: | $(filter-out mingw-w64-% musl-% glibc-% linux-% netbsd-% openbsd-%,$(SRC_DIRS))
else ifneq ($(findstring netbsd,$(TARGET)),)
extract_all: | $(filter-out mingw-w64-% musl-% glibc-% linux-% freebsd-% openbsd-%,$(SRC_DIRS))
else ifneq ($(findstring openbsd,$(TARGET)),)
extract_all: | $(filter-out mingw-w64-% musl-% glibc-% linux-% freebsd-% netbsd-%,$(SRC_DIRS))
else ifneq ($(findstring musl,$(TARGET)),)
extract_all: | $(filter-out mingw-w64-% glibc-% freebsd-% netbsd-% openbsd-%,$(SRC_DIRS))
else ifneq ($(findstring gnu,$(TARGET))$(findstring glibc,$(TARGET)),)
extract_all: | $(filter-out mingw-w64-% musl-% freebsd-% netbsd-% openbsd-%,$(SRC_DIRS))
else
...
$(BUILD_DIR)/config.mak: | $(BUILD_DIR)
	...
	$(if $(NETBSD_VER),$(if $(findstring x86_64,$(TARGET)),"NETBSD_SRCDIR = $(REL_TOP)/netbsd-$(NETBSD_VER)-amd64",$(if $(findstring aarch64,$(TARGET)),"NETBSD_SRCDIR = $(REL_TOP)/netbsd-$(NETBSD_VER)-evbarm-aarch64",$(if $(findstring i386,$(TARGET)),"NETBSD_SRCDIR = $(REL_TOP)/netbsd-$(NETBSD_VER)-i386",$(if $(findstring i586,$(TARGET)),"NETBSD_SRCDIR = $(REL_TOP)/netbsd-$(NETBSD_VER)-i386",$(if $(findstring i686,$(TARGET)),"NETBSD_SRCDIR = $(REL_TOP)/netbsd-$(NETBSD_VER)-i386",$(if $(findstring sparc64,$(TARGET)),"NETBSD_SRCDIR = $(REL_TOP)/netbsd-$(NETBSD_VER)-sparc64"))))))) \
	$(if $(OPENBSD_VER),$(if $(findstring x86_64,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-amd64",$(if $(findstring aarch64,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-arm64",$(if $(findstring armv7,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-armv7",$(if $(findstring i386,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-i386",$(if $(findstring i586,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-i386",$(if $(findstring i686,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-i386",$(if $(findstring riscv64,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-riscv64",$(if $(findstring powerpc64,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-powerpc64",$(if $(findstring sparc64,$(TARGET)),"OPENBSD_SRCDIR = $(REL_TOP)/openbsd-$(OPENBSD_VER)-sparc64")))))))))) \
